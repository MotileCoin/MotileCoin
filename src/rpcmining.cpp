...
Value checkkernel(const Array& params, bool fHelp)
{
	...
	if (vNodes.empty())
		throw JSONRPCError(-9, "Motile is not connected!");

	if (IsInitialBlockDownload())
		throw JSONRPCError(-10, "Motile is downloading blocks...");
...
Value mine(const Array& params, bool fHelp)
{
	...
	if (vNodes.empty())
		throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Motile is not connected!");

	if (IsInitialBlockDownload())
		throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Motile is downloading blocks...");
