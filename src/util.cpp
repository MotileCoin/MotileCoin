...
int nAnonymizeMotileAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Motile
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Motile
    // Mac: ~/Library/Application Support/Motile
    // Unix: ~/.Motile
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Motile";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Motile.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Motiled.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Motile will not work properly.");
