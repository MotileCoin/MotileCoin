...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeMotileAmount"))
        settings.setValue("nAnonymizeMotileAmount", 1000);
    nAnonymizeMotileAmount = settings.value("nAnonymizeMotileAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeMotileAmount"))
        SoftSetArg("-anonymizeMotileamount", settings.value("nAnonymizeMotileAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeMotileAmount:
            return QVariant(nAnonymizeMotileAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeMotileAmount:
            nAnonymizeMotileAmount = value.toInt();
            settings.setValue("nAnonymizeMotileAmount", nAnonymizeMotileAmount);
            emit AnonymizeMotileAmountChanged(nAnonymizeMotileAmount);
            break;
        default:
            break;
        }
