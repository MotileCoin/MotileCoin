...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeMotileAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeMotileAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeMotileAmount = strAnonymizeMotileAmount.remove(strAnonymizeMotileAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMotileAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeMotileAmount*COIN) nMaxToAnonymize = nAnonymizeMotileAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeMotileAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeMotileAmount));
        strAnonymizeMotileAmount = strAnonymizeMotileAmount.remove(strAnonymizeMotileAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMotileAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
